<template>
  <div>
    <h1>Newcastle City Juniors - Football Talk-In 2025</h1>

    <div class="mx-auto max-w-5xl">
      <img src="https://images.ctfassets.net/yv1glylq11v9/5VA7slU0B5CuHDsOo6xJZ0/749b4eb6cc65ac0da71e6198b464261d/image_4.png">
    </div>

    <p class="py-8">Newcastle City Juniors are happy to announce our upcoming Football Talk-In 2025!</p>

    <div class="pb-8">
      <ul class="list-disc list-inside">
        <li>Friday, 30th May 2025</li>
        <li>An evening with former Newcastle United striker Les Ferdinand</li>
        <li>Pies and Peas included</li>
        <li>Entry - £400 (table of 10 people) or £45 per person</li>
        <li>Raffle Prizes</li>
...
export default {
  name: 'PageNewsAndEventsFootyTalkIn2025',
  ...
  data: () => ({
    ...
    ticketOptions: [
      { key: 'table', value: 'Table of 10 (£400)' },
      { key: 'individual', value: 'Individual (£45)' },
    ],
    stripe: {
      client: null,
      elements: {
  ...
  computed: {
    paymentAmount() {
      return this.form.ticketOption === 'table' ? 40000 : 4500;
    },
  },
  ...
  watch: {
    'form.ticketOption': function updatePaymentIntent() {
      this.$nextTick(async () => {
        await this.$axios.post('/api/stripe/payment-intents/footy-talk-in-2025', {
    ...
    async showSignupForm() {
      if (!this.formActive) {
        ...
        try {
          ...
          const { data: { paymentIntent } } = await this.$axios.post('/api/stripe/payment-intents/footy-talk-in-2025', {
            amount: this.paymentAmount,
            form: this.form,
            paymentIntentId: this.stripe.payment.intent,
          });
